...
You can start a asynchronous test with a call to *SSUnit.startAsync()*. This will generate a hook which can be used to declare that the test has successfully completed. You might notice that no asynchronous calls appear to be made here. This is because SSUnit is built on top of Promises, a MooTools library for simplfying request heavy code as well as enabling a simple form of delayed evaluation. Please look at http://github.com/ShiftSpace/promises/tree/master for a lengthier exposition.
